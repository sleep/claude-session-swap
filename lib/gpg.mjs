...
// gpg is absent (or too old): the user can fix it, so setup flows report it
// as guidance rather than as a crash.
export class GpgMissingError extends Error {}

// Keys are never fetched from the network: a message signed by an unknown
// key must simply fail, not trigger a keyserver or WKD lookup.
export const BASE_ARGS = ['--batch', '--no-tty', '--no-auto-key-retrieve', '--no-auto-key-locate'];
...
function run(args, { gpgBin, env = process.env, input } = {}) {
  ...
  const r = spawnSync(bin, [...BASE_ARGS, ...args], { env, input, encoding: 'utf8', maxBuffer: 16 * 1024 * 1024, timeout: RUN_TIMEOUT_MS });
  if (r.error) throw new GpgError(`could not run ${bin}: ${r.error.message}`);
  if (r.signal) throw new GpgError(`${bin} did not finish within ${RUN_TIMEOUT_MS / 1000}s`);
  return r;
}
...
export async function ensureGpg({ rl, out = console.log, env = process.env, platform = process.platform } = {}) {
  ...
  if (cmd && rl) {
    ...
    if (answer === 'y' || answer === 'yes') {
      ...
      if (r.status !== 0) throw new GpgMissingError(`"${cmd}" failed; install gpg and re-run setup`);
      found = findGpg({ env });
      if (found) return found;
    }
  }
  throw new GpgMissingError('gpg is not installed; install it and re-run setup');
}
...
// Without a terminal there is nobody to answer a pinentry, so a key whose
// passphrase the agent has not cached fails at once instead of blocking a
// statusline script forever.
export function createPgp({ fingerprint, gpgBin, env = process.env, interactive = !!process.stdin.isTTY }) {
  if (!FPR_RE.test(fingerprint)) throw new GpgError(`not a key fingerprint: ${fingerprint}`);
  const opts = { gpgBin, env };
  const pin = interactive ? [] : ['--pinentry-mode', 'error'];
  return {
    fingerprint,
    exportPublicKey() {
    ...
    sign(text) {
      const r = run([...pin, '--detach-sign', '--armor', '--local-user', fingerprint], { ...opts, input: text });
      if (r.status !== 0) throw new GpgError(`signing failed: ${r.stderr.trim()}`);
      return r.stdout;
    },
    encrypt(text) {
      const r = run(
        [...pin, '--sign', '--encrypt', '--armor', '--trust-model', 'always', '--local-user', fingerprint, '--recipient', fingerprint],
        { ...opts, input: text },
      );
    ...
    decrypt(armored) {
      const r = run([...pin, '--decrypt', '--status-fd', '2', '--max-output', String(4 * 1024 * 1024)], { ...opts, input: armored });
